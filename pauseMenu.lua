...
function pauseMenu.onPause(eventToken)
	if not eventToken.cancelled then -- Just in case make sure this event isn't already flagged as cancelled
		...
		-- [MODIFIED BY CHROMANYAN] the music pausing disrespects when the stream is siezed, so it has been turned off
		--[[
		...
		-- we can pause siezed music though, this will do nothing if the current music isn't siezed
		Audio.MusicPause()
...
pauseMenu.options = {
	...
	{name = "Restart",    action = pauseMenu.Restart,   confirm = true,  text = "This will reload the level and unsaved progress will be lost."},
	{name = "Exit To Hub", action = pauseMenu.ExitLevel, confirm = true,  text = "This will exit the level and unsaved progress will be lost."},
	{name = "Quit Game",  action = pauseMenu.Quit,      confirm = true,  text = "This will quit the game and unsaved progress will be lost."}
}
