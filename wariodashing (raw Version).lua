...
gfxoffsetY = 8, 	-- the amount of pixels the graphic is rendered below the place it actually would; used so it can actually be renderered on grounded
gfxwidth = 50,		-- the width one frame takes up on a spritesheet
gfxheight = 50, 	-- the height one frame takes up on a spritesheet
groundframes = 4, 	-- the amount of frames the ground dashing animation has
airframes = 3,		-- the amount of frames the air dashing animation has
}
...
function wariodashing.onTick()
	...
	if wariodashing.dash then					-- do the dashing code
		...
		if player:isOnGround() then			-- plays the ground animation
			...
			if (frame < wariodashing.groundframes - 1) and frametimer > 1 then	-- go up one frame
				frame = frame + 1
				frametimer = 0
			elseif (frame >= wariodashing.groundframes - 1) and frametimer > 1 then	-- get back to default frame
				frame = 0
				frametimer = 0
			elseif frametimer <= 1 then
				frametimer = frametimer + 1								-- counts up the frametimer until it reaches 2, then it gets back to 0
			end
			if frame > wariodashing.groundframes - 1 then	-- so the player isn't invisible for a frame when changing from air dash to ground dash in a bad moment (Necessary as the animation for ground and air are varying in length)
				frame = 0												
			end
		else															-- plays the air animation
			inair = 1													-- the player is in the air (note for the animation, hence it's in integer values)
			if (frame < wariodashing.airframes - 1) and frametimer > 1 then			-- go up one frame
				frame = frame + 1
				frametimer = 0
			elseif (frame >= wariodashing.airframes - 1) and frametimer > 1 then		-- get back to default frame
				frame = 0
				frametimer = 0
			elseif frametimer <= 1 then
				frametimer = frametimer + 1								-- counts up the frametimer until it reaches 2, then it gets back to 0
			end
			if frame > wariodashing.airframes - 1 then	-- so the player isn't invisible for a frame when changing from ground dash to air dash in a bad moment (Necessary as the animation for ground and air are varying in length)
				frame = 0
			end
		end
...
function wariodashing.onDraw()
	if wariodashing.dash then
		...
		Graphics.drawBox{
			...
			y            = player.y + (player.height / 2) - wariodashing.gfxoffsetY,
