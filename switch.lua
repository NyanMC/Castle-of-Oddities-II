...
local clock = Sprite{
	...
	texture = Graphics.loadImageResolved "clock.png",
}
...
switch.shouldPause = true
switch.overlyDramatic = false

lastFullSecond = 0

function switch.setMaxTime(val)
	--if not active then -- another modification by nyan
		timer = val
	--end
end
...
function switch.getRemainingTime() -- added by nyan, i needed it for a level, sorry
	return timer
end
	...
	if active then
		...
		elseif start >= 6 then
			...
			if not Misc.isPaused() then
				...
				if switch.overlyDramatic and not (lastFullSecond == math.floor(timer)) and timer > 0 then
					lastFullSecond = math.floor(timer)
					SFX.play(Misc.resolveFile("WarTimer.ogg"))
					Defines.earthquake = math.max(((20 - lastFullSecond) / 2), 0)
				end
			end
		end
...
function switch.onTickEnd()
	if active then
		...
		if earthquake_timer > 64 then
			if not switch.overlyDramatic then
			...
			for k,p in ipairs(Player.get()) do
				if p.deathTimer <= 0 then
					if timer <= 0 then
						if not dying then
							Audio.SeizeStream(-1)
							Audio.MusicOpen("coin_loss.ogg")
							Audio.MusicPlay()
							dying = true
						end
						currentlyBoss = true
						if switch.overlyDramatic or Misc.score() < 20 then
							Misc.score(Misc.score() * -1)
							p:kill()
							Explosion.spawn(p.x + 0.5 * p.width, p.y + 0.5 * p.height, 3)
							Defines.earthquake = 30
							dead = true
						else
							Misc.score(-20)
						end
